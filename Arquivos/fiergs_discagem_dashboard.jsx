...
const statusNegocio = [
  ...
  { name: "Engano", value: 183, color: "#94A3B8" },
  { name: "Fora do Perfil", value: 173, color: "#64748B" },
  { name: "Lig. Muda", value: 102, color: "#475569" },
  { name: "Não Tabulado", value: 78, color: "#334155" },
...
const mailingComparativo = [
  { label: "Base Mailing Total", value: 64013, color: "#475569" },
...
function MetricCard({ label, value, sub, color = GREEN, alert, size = "normal" }) {
  return (
    <div style={{
      background: CARD,
      border: `1px solid ${alert ? color + "88" : BORDER}`,
      borderLeft: `3px solid ${color}`,
      padding: size === "big" ? "18px 20px" : "12px 16px",
      borderRadius: 4,
    }}>
      <div style={{ color: "#64748B", fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 3 }}>{label}</div>
      <div style={{ color, fontSize: size === "big" ? 30 : 20, fontWeight: 700, lineHeight: 1.1, fontFamily: "monospace" }}>{value}</div>
...
function AlertBox({ type, children }) {
  ...
  return (
    <div style={{ padding: "10px 14px", background: c + "11", border: `1px solid ${c}33`, borderLeft: `3px solid ${c}`, borderRadius: 4, fontSize: 12, color: "#94A3B8", lineHeight: 1.6 }}>
      {children}
    </div>
  );
}
            ...
            return (
              <div key={i} style={{ flex: 1, textAlign: "center", position: "relative" }}>
                <div style={{
                  ...
                  <div style={{ fontSize: 10, color: "#64748B", marginBottom: 6, lineHeight: 1.3 }}>{step.label}</div>
                  ...
                  {i > 0 && (
                    <div style={{ fontSize: 10, color: "#475569", marginTop: 2 }}>▼ {pctAnterior}% ant.</div>
                  )}
                </div>
                {i < arr.length - 1 && (
                  <div style={{ position: "absolute", right: -8, top: "50%", transform: "translateY(-50%)", color: "#334155", fontSize: 14, zIndex: 2 }}>▶</div>
                )}
              </div>
            );
          })}
              ...
              <div key={i} style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                  <span style={{ color: "#94A3B8", fontSize: 12 }}>
                    <span style={{ color: "#475569", fontFamily: "monospace", marginRight: 6 }}>[{item.code}]</span>
                    {item.desc}
                  </span>
                  <span style={{ color: item.code === "1" ? GREEN : item.code === "147" || item.code === "128" ? YELLOW : "#64748B", fontSize: 12, fontFamily: "monospace", fontWeight: 700 }}>
                    {item.pct}%
                    <span style={{ color: "#475569", fontWeight: 400 }}> ({item.count.toLocaleString("pt-BR")})</span>
                  </span>
                </div>
                <div style={{ height: 5, background: "#0A1520", borderRadius: 3 }}>
                  <div style={{ width: item.pct + "%", height: "100%", background: item.code === "1" ? GREEN : item.code === "147" || item.code === "128" ? YELLOW : "#334155", borderRadius: 3 }} />
                </div>
              </div>
            ))}
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ padding: "14px 16px", background: "#0A1520", borderRadius: 4, flex: 1 }}>
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
        ...
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={porDia} barCategoryGap="25%">
            ...
            <YAxis yAxisId="right" orientation="right" tick={{ fill: "#94A3B8", fontSize: 10 }} domain={[0, 80]} />
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={porHora}>
            ...
            <YAxis domain={[40, 65]} tick={{ fill: "#94A3B8", fontSize: 10 }} unit="%" />
            <Tooltip contentStyle={{ background: NAVY, border: `1px solid ${BORDER}` }} labelStyle={{ color: LIGHT }} formatter={(v) => [v + "%", "Taxa Contato"]} />
            <ReferenceLine y={54.1} stroke="#475569" strokeDasharray="4 4" label={{ value: "Média: 54,1%", fill: "#64748B", fontSize: 11 }} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginTop: 12 }}>
          <div style={{ background: GREEN + "18", border: `1px solid ${GREEN}44`, borderRadius: 4, padding: "10px 14px" }}>
            ...
            <div style={{ color: "#64748B", fontSize: 11 }}>Acima de 56% de contato — priorizar volume nesses horários</div>
          </div>
          <div style={{ background: YELLOW + "18", border: `1px solid ${YELLOW}44`, borderRadius: 4, padding: "10px 14px" }}>
            ...
            <div style={{ color: "#64748B", fontSize: 11 }}>Taxa cai para 45,9% — menor eficiência operacional</div>
          </div>
          <div style={{ background: BLUE + "18", border: `1px solid ${BLUE}44`, borderRadius: 4, padding: "10px 14px" }}>
            ...
            <div style={{ color: "#64748B", fontSize: 11 }}>Redirecionar volume para 14h–16h e economizar tentativas</div>
          </div>
        </div>
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={statusNegocio} layout="vertical" barCategoryGap="15%">
              ...
              <YAxis dataKey="name" type="category" tick={{ fill: "#94A3B8", fontSize: 11 }} width={115} />
              <Tooltip contentStyle={{ background: NAVY, border: `1px solid ${BORDER}` }} labelStyle={{ color: LIGHT }} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ background: "#0A1520", borderRadius: 4, padding: 14 }}>
              <div style={{ color: "#64748B", fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", marginBottom: 8 }}>BREAKDOWN DE NEGÓCIO</div>
              {[
                ...
                { label: "Fora do Perfil / Engano", value: 356, pct: "20,8%", color: "#64748B" },
                ...
                <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: `1px solid ${BORDER}` }}>
                  ...
                  <span style={{ color: item.color, fontFamily: "monospace", fontSize: 12, fontWeight: 700 }}>{item.value} <span style={{ color: "#475569", fontWeight: 400 }}>({item.pct})</span></span>
                </div>
              ))}
            </div>
            <AlertBox type="yellow">
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {[["interesse", "Interesse"], ["convReal", "Conv. Real"], ["tma", "TMA"]].map(([k, l]) => (
              <button key={k} onClick={() => setSort(k)} style={{
                ...
                background: sort === k ? GREEN + "22" : "transparent", color: sort === k ? GREEN : "#64748B",
          ...
          <thead>
            <tr style={{ background: "#0A1520" }}>
              {["#", "Agente", "Atendidos", "Conv. Real", "Conv. Real %", "TMA (s)", "Interesses", "Oportunid.", "Conv. Rate"].map(h => (
                <th key={h} style={{ padding: "8px 10px", color: "#64748B", textAlign: "left", borderBottom: `1px solid ${BORDER}`, fontSize: 11 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {sorted.map((ag, i) => {
              ...
              return (
                <tr key={ag.id} style={{ borderBottom: `1px solid ${BORDER}`, background: isTop ? GREEN + "08" : "transparent" }}>
                  <td style={{ padding: "8px 10px", color: isTop ? GREEN : "#475569", fontWeight: isTop ? 700 : 400, fontFamily: "monospace" }}>
                    {isTop ? ["🥇", "🥈", "🥉"][i] : i + 1}
                  </td>
                  <td style={{ padding: "8px 10px", color: LIGHT, fontWeight: 600 }}>{ag.nome}</td>
                  <td style={{ padding: "8px 10px", color: "#94A3B8", fontFamily: "monospace" }}>{ag.atendidos}</td>
                  <td style={{ padding: "8px 10px", color: BLUE, fontFamily: "monospace", fontWeight: 700 }}>{ag.convReal}</td>
                  <td style={{ padding: "8px 10px", color: "#64748B", fontFamily: "monospace" }}>{convPct}%</td>
                  ...
                  <td style={{ padding: "8px 10px", color: ag.oportunidade > 0 ? "#A855F7" : "#475569", fontWeight: 700, fontFamily: "monospace" }}>{ag.oportunidade}</td>
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={agentes.sort((a, b) => b.interesse - a.interesse)} barCategoryGap="20%">
            <CartesianGrid strokeDasharray="3 3" stroke={BORDER} />
            <XAxis dataKey="nome" tick={{ fill: "#94A3B8", fontSize: 10 }} angle={-15} textAnchor="end" height={50} />
              ...
              {pilar.items.map((row, i) => {
                ...
                return (
                  <tr key={i} style={{ borderBottom: `1px solid ${BORDER}`, background: i % 2 === 0 ? "transparent" : "#0A1520" }}>
                    ...
                    <td style={{ padding: "8px 10px", color: "#94A3B8" }}>{row.benchmark}</td>
                    ...
                    <td style={{ padding: "8px 10px", color: "#64748B", fontSize: 11 }}>{row.nota}</td>
                  </tr>
                );
              })}
            </tbody>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {acoes.map((a, i) => (
        <div key={i} style={{ background: CARD, border: `1px solid ${BORDER}`, borderLeft: `4px solid ${a.cor}`, borderRadius: 4, padding: 18 }}>
          ...
          <div style={{ color: "#94A3B8", fontSize: 12, marginBottom: 10 }}><strong style={{ color: "#64748B" }}>Problema:</strong> {a.problema}</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={{ background: "#0A1520", borderRadius: 3, padding: "10px 12px" }}>
              <div style={{ color: "#475569", fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", marginBottom: 4 }}>AÇÃO</div>
...
export default function App() {
  ...
  return (
    <div style={{ background: NAVY, minHeight: "100vh", fontFamily: "'Inter', -apple-system, sans-serif", color: LIGHT }}>
      {/* Header */}
      <div style={{ borderBottom: `1px solid ${BORDER}`, padding: "14px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ background: "#1E3A5F", color: BLUE, fontWeight: 800, fontSize: 11, padding: "3px 10px", borderRadius: 2, letterSpacing: "0.08em", border: `1px solid ${BLUE}44` }}>FIERGS + DISCAGEM</div>
            <div style={{ background: "#0A2010", color: GREEN, fontWeight: 800, fontSize: 11, padding: "3px 10px", borderRadius: 2, letterSpacing: "0.08em", border: `1px solid ${GREEN}44` }}>BR + PRODUTIVO</div>
            <h1 style={{ color: LIGHT, fontSize: 17, fontWeight: 700, margin: 0 }}>Análise Operacional — Visão Apartada</h1>
          </div>
          <div style={{ color: "#475569", fontSize: 11, marginTop: 4 }}>Fila: 1063 - Ativo · 01/06–05/06/2026 · 4 dias úteis · 10 agentes · RS 100% do mailing</div>
        </div>
        <div style={{ display: "flex", gap: 20, textAlign: "right" }}>
          <div>
            <div style={{ color: GREEN, fontSize: 20, fontWeight: 800, fontFamily: "monospace" }}>293</div>
            <div style={{ color: "#475569", fontSize: 10 }}>leads positivos</div>
          </div>
          <div>
            <div style={{ color: YELLOW, fontSize: 20, fontWeight: 800, fontFamily: "monospace" }}>72.113</div>
            <div style={{ color: "#475569", fontSize: 10 }}>tentativas</div>
          </div>
          <div>
            <div style={{ color: BLUE, fontSize: 20, fontWeight: 800, fontFamily: "monospace" }}>36,4%</div>
            <div style={{ color: "#475569", fontSize: 10 }}>mailing coberto</div>
          </div>
        </div>
      </div>

      {/* Tabs */}
      <div style={{ borderBottom: `1px solid ${BORDER}`, padding: "0 28px", display: "flex", gap: 0 }}>
        {TABS.map((t, i) => (
          <button key={i} onClick={() => setActive(i)} style={{
            padding: "13px 18px", background: "transparent", border: "none",
            borderBottom: `2px solid ${i === active ? GREEN : "transparent"}`,
            color: i === active ? GREEN : "#64748B", cursor: "pointer", fontSize: 12,
            fontWeight: i === active ? 700 : 400, transition: "all 0.15s"
          }}>{t}</button>
        ))}
      </div>

      <div style={{ padding: "24px 28px" }}>{tabComponents[active]}</div>

      <div style={{ borderTop: `1px solid ${BORDER}`, padding: "10px 28px", display: "flex", justifyContent: "space-between", fontSize: 11, color: "#334155" }}>
        <span>Dados: Discagem_Fila.csv · Mailing_FIERGS_202606.xlsx · Fila 1063 - Ativo</span>
        <span>Gerado em {new Date().toLocaleDateString("pt-BR")} · Operação: 01–05/jun/2026</span>
      </div>
    </div>
  );
}
