...
function MetricCard({ label, value, sub, color = GREEN, size = "normal" }) {
  return (
    <div style={{
      ...
      padding: size === "big" ? "20px 24px" : "14px 18px",
      borderRadius: 4,
    }}>
      <div style={{ color: "#94A3B8", fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4 }}>{label}</div>
      <div style={{ color: color, fontSize: size === "big" ? 32 : 22, fontWeight: 700, lineHeight: 1.1, fontFamily: "monospace" }}>{value}</div>
...
function SectionTitle({ n, title }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
      <div style={{ background: GREEN, color: NAVY, width: 28, height: 28, borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 13, flexShrink: 0 }}>{n}</div>
...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ background: color + "22", color, border: `1px solid ${color}44`, borderRadius: 3, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>{label}</span>
  );
}
      ...
      <thead>
        <tr style={{ background: "#0A1520" }}>
          {["KPI", "Fórmula", "Benchmark", "Meta", "Status", "Observação"].map(h => (
            <th key={h} style={{ padding: "8px 10px", color: "#64748B", textAlign: "left", fontWeight: 600, borderBottom: `1px solid ${BORDER}`, fontSize: 11 }}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {kpis.map((k, i) => (
          <tr key={i} style={{ borderBottom: `1px solid ${BORDER}`, background: i % 2 === 0 ? "transparent" : "#0A1520" }}>
            <td style={{ padding: "8px 10px", color: LIGHT, fontWeight: 600 }}>{k.nome}</td>
            <td style={{ padding: "8px 10px", color: "#94A3B8", fontFamily: "monospace", fontSize: 11 }}>{k.formula}</td>
            ...
            <td style={{ padding: "8px 10px", color: "#64748B", fontSize: 11 }}>{k.obs}</td>
          </tr>
        ))}
      </tbody>
    </table>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
          <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Distribuição por Porte</div>
          <div style={{ display: "flex", gap: 16, marginBottom: 16 }}>
            {porteData.map(p => (
              <div key={p.name} style={{ flex: 1, textAlign: "center" }}>
                ...
                <div style={{ color: "#64748B", fontSize: 11 }}>{p.value.toLocaleString("pt-BR")}</div>
              </div>
            ))}
          </div>
          <div style={{ height: 12, background: "#0A1520", borderRadius: 2, display: "flex", overflow: "hidden" }}>
            {porteData.map(p => <div key={p.name} style={{ width: p.pct + "%", background: p.color }} />)}
          </div>
          <div style={{ marginTop: 12, padding: "8px 12px", background: "#0A1520", borderRadius: 3, fontSize: 11, color: "#94A3B8" }}>
        ...
        {/* Capital Social */}
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
          <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Capital Social — Faixas</div>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={capitalData} barCategoryGap="30%">
              <CartesianGrid strokeDasharray="3 3" stroke={BORDER} />
              <XAxis dataKey="faixa" tick={{ fill: "#94A3B8", fontSize: 11 }} />
              <YAxis tick={{ fill: "#94A3B8", fontSize: 10 }} />
              <Tooltip contentStyle={{ background: NAVY, border: `1px solid ${BORDER}`, borderRadius: 4 }} labelStyle={{ color: LIGHT }} itemStyle={{ color: GREEN }} />
      ...
      {/* Segmento × porte */}
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
        <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Top Segmentos × Porte (IBGE)</div>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={segmentoData} barCategoryGap="25%">
            <CartesianGrid strokeDasharray="3 3" stroke={BORDER} />
            <XAxis dataKey="seg" tick={{ fill: "#94A3B8", fontSize: 11 }} />
            <YAxis tick={{ fill: "#94A3B8", fontSize: 10 }} />
            <Tooltip contentStyle={{ background: NAVY, border: `1px solid ${BORDER}` }} labelStyle={{ color: LIGHT }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
          <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14 }}>Distribuição por Região (RS)</div>
        ...
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
          <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14 }}>Disponibilidade de Canais de Contato</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 8 }}>
            {[
              ...
              { label: "WhatsApp Mapeado", value: 0, pct: 0, color: "#64748B" },
            ].map(item => (
              <div key={item.label}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ color: "#94A3B8", fontSize: 12 }}>{item.label}</span>
                  <span style={{ color: item.color, fontSize: 12, fontWeight: 700, fontFamily: "monospace" }}>{item.pct}% <span style={{ color: "#64748B", fontWeight: 400 }}>({item.value.toLocaleString("pt-BR")})</span></span>
                </div>
          ...
          <div style={{ marginTop: 14, padding: "8px 12px", background: "#0A1520", borderRadius: 3, fontSize: 11, color: "#94A3B8" }}>
...
function TabCnaes() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      {/* Top 10 CNAE bar */}
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
        <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Top 10 CNAEs — Volume na Carteira</div>
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={topCnaes} layout="vertical" barCategoryGap="15%">
            <CartesianGrid strokeDasharray="3 3" stroke={BORDER} />
            <XAxis type="number" tick={{ fill: "#94A3B8", fontSize: 10 }} />
            <YAxis dataKey="desc" type="category" tick={{ fill: "#94A3B8", fontSize: 10 }} width={195} />
            <Tooltip contentStyle={{ background: NAVY, border: `1px solid ${BORDER}` }} labelStyle={{ color: LIGHT }} />
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
        <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>
          Enriquecimento CNAE — Carteira vs. Universo RS
          <span style={{ marginLeft: 12, color: "#475569", fontWeight: 400 }}>Fonte: Receita Federal / DataSebrae estimado</span>
        </div>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ background: "#0A1520" }}>
          ...
          <tbody>
            {cnaeEnriquecimento.map((r, i) => {
              ...
              return (
                <tr key={i} style={{ borderBottom: `1px solid ${BORDER}`, background: i % 2 === 0 ? "transparent" : "#0A1520" }}>
                  <td style={{ padding: "8px 10px", color: "#94A3B8", fontFamily: "monospace", fontSize: 11 }}>{r.cnae}</td>
                  ...
                  <td style={{ padding: "8px 10px", color: "#94A3B8", fontFamily: "monospace" }}>{r.universoRS.toLocaleString("pt-BR")}</td>
                  <td style={{ padding: "8px 10px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ width: 48, height: 6, background: "#0A1520", borderRadius: 3 }}>
                  ...
                  <td style={{ padding: "8px 10px", color: "#64748B", fontSize: 11 }}>{r.topUF}</td>
                  <td style={{ padding: "8px 10px", color: "#64748B", fontSize: 11 }}>{r.topCidade}</td>
                </tr>
              );
            })}
          </tbody>
        </table>
        <div style={{ marginTop: 12, padding: "10px 14px", background: "#0A1520", borderRadius: 3, fontSize: 11, color: "#94A3B8", borderLeft: `3px solid ${YELLOW}` }}>
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
        <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Insights de Mercado Externo Aplicáveis</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {[
            ...
            <div key={i} style={{ background: "#0A1520", border: `1px solid ${item.color}33`, borderRadius: 4, padding: 14 }}>
              ...
              <div style={{ color: "#64748B", fontSize: 11, borderTop: `1px solid ${BORDER}`, paddingTop: 6 }}>→ {item.aplicacao}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        {/* Top cidades */}
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
          <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Top 10 Cidades — Leads na Carteira</div>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={cidadesData} layout="vertical" barCategoryGap="15%">
              <CartesianGrid strokeDasharray="3 3" stroke={BORDER} />
              <XAxis type="number" tick={{ fill: "#94A3B8", fontSize: 10 }} />
              <YAxis dataKey="cidade" type="category" tick={{ fill: "#94A3B8", fontSize: 11 }} width={120} />
              <Tooltip contentStyle={{ background: NAVY, border: `1px solid ${BORDER}` }} labelStyle={{ color: LIGHT }} />
        ...
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
          <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14 }}>Índice de Cobertura por Região RS</div>
          <div style={{ color: "#475569", fontSize: 11, marginBottom: 14 }}>Fórmula: (Registros Carteira por Região ÷ Total MPEs Elegíveis por Região) × 100</div>
          {[
            ...
            <div key={item.regiao} style={{ marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                <span style={{ color: "#94A3B8", fontSize: 12 }}>{item.regiao}</span>
                <span style={{ color: item.color, fontSize: 12, fontFamily: "monospace", fontWeight: 700 }}>{item.pct}%
                  <span style={{ color: "#475569", fontWeight: 400 }}> ({item.carteira.toLocaleString("pt-BR")} / {item.universo.toLocaleString("pt-BR")})</span>
                </span>
              </div>
              <div style={{ height: 6, background: "#0A1520", borderRadius: 3 }}>
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
        <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>
          ...
          <span style={{ marginLeft: 12, color: "#475569", fontWeight: 400 }}>Empresas elegíveis não cobertas pela carteira atual</span>
        </div>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ background: "#0A1520" }}>
              <th style={{ padding: "8px 12px", color: "#64748B", textAlign: "left", borderBottom: `1px solid ${BORDER}`, fontSize: 11 }}>Região</th>
              {["Construção", "Comércio Veíc.", "Indústria Möv.", "Engenharia", "Elétrica"].map(h => (
                <th key={h} style={{ padding: "8px 10px", color: "#64748B", textAlign: "center", borderBottom: `1px solid ${BORDER}`, fontSize: 11 }}>{h}</th>
              ))}
              <th style={{ padding: "8px 10px", color: "#64748B", textAlign: "center", borderBottom: `1px solid ${BORDER}`, fontSize: 11 }}>Score</th>
            </tr>
          </thead>
          <tbody>
            {[
              ...
              <tr key={row.r} style={{ borderBottom: `1px solid ${BORDER}`, background: i % 2 === 0 ? "transparent" : "#0A1520" }}>
        ...
        <div style={{ marginTop: 12, padding: "10px 14px", background: "#0A1520", borderRadius: 3, fontSize: 11, color: "#94A3B8", borderLeft: `3px solid ${RED}` }}>
...
function TabKPIs() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ background: CARD, border: `1px solid ${RED}44`, borderRadius: 4, padding: 16 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {pilares.map((p, i) => (
          <button key={i} onClick={() => setPilar(i)} style={{
            padding: "10px 20px", borderRadius: 3, border: `1px solid ${i === pilar ? p.color : BORDER}`,
            background: i === pilar ? p.color + "22" : "transparent", color: i === pilar ? p.color : "#64748B",
      ...
      {/* Funil esperado */}
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
        <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Funil de Conversão Projetado — Base 64.013 Registros</div>
        <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
          {[
            { label: "Carteira Total", value: 64013, pct: "100%", color: "#475569" },
            ...
            <div key={i} style={{ flex: 1, textAlign: "center", position: "relative" }}>
              <div style={{
                background: step.color + "22", border: `1px solid ${step.color}66`, borderRadius: 4,
                padding: "14px 8px", margin: "0 2px", height: "100%", boxSizing: "border-box"
              }}>
                <div style={{ fontSize: 11, color: "#64748B", marginBottom: 6 }}>{step.label}</div>
              ...
              {i < arr.length - 1 && (
                <div style={{ position: "absolute", right: -8, top: "50%", transform: "translateY(-50%)", color: "#64748B", fontSize: 16, zIndex: 2 }}>▶</div>
              )}
            </div>
          ))}
        </div>
        <div style={{ marginTop: 14, padding: "10px 14px", background: "#0A1520", borderRadius: 3, fontSize: 11, color: "#94A3B8", borderLeft: `3px solid ${GREEN}` }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {recomendacoes.map((r, i) => (
        <div key={i} style={{ background: CARD, border: `1px solid ${BORDER}`, borderLeft: `4px solid ${r.prioridade.includes("CRÍTICA") ? RED : r.prioridade.includes("ALTA") ? YELLOW : GREEN}`, borderRadius: 4, padding: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
            ...
            <span style={{ background: BORDER, color: "#94A3B8", borderRadius: 3, padding: "2px 8px", fontSize: 11 }}>Seg. {r.segmento}</span>
          </div>
          <div style={{ color: "#94A3B8", fontSize: 13, marginBottom: 10, lineHeight: 1.5 }}>{r.justificativa}</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={{ background: "#0A1520", borderRadius: 3, padding: "10px 14px" }}>
              <div style={{ color: "#475569", fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", marginBottom: 4 }}>AÇÃO RECOMENDADA</div>
              <div style={{ color: LIGHT, fontSize: 12 }}>{r.acao}</div>
            </div>
            <div style={{ background: "#0A1520", borderRadius: 3, padding: "10px 14px" }}>
              <div style={{ color: "#475569", fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", marginBottom: 4 }}>IMPACTO ESPERADO</div>
      ...
      {/* Roadmap visual */}
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: 20 }}>
        <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Roadmap de Implantação</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          {[
            ...
            <div key={i} style={{ background: "#0A1520", border: `1px solid ${fase.color}44`, borderRadius: 4, padding: 16 }}>
...
export default function App() {
  ...
  return (
    <div style={{ background: NAVY, minHeight: "100vh", fontFamily: "'Inter', -apple-system, sans-serif", color: LIGHT }}>
      {/* Header */}
      <div style={{ borderBottom: `1px solid ${BORDER}`, padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ background: GREEN, color: NAVY, fontWeight: 800, fontSize: 12, padding: "3px 10px", borderRadius: 2, letterSpacing: "0.08em" }}>BR + PRODUTIVO</div>
            <h1 style={{ color: LIGHT, fontSize: 18, fontWeight: 700, margin: 0 }}>Análise de Mailing — Ação Ativa de Discagem</h1>
          </div>
          <div style={{ color: "#475569", fontSize: 12, marginTop: 4 }}>Carteira: 25/mai/26 · Estado: Rio Grande do Sul · 64.013 registros · Situação: 100% Ativa</div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ color: GREEN, fontSize: 22, fontWeight: 800, fontFamily: "monospace" }}>64.013</div>
          <div style={{ color: "#475569", fontSize: 11 }}>registros · ~11% cobertura RS</div>
        </div>
      </div>
      ...
      <div style={{ borderBottom: `1px solid ${BORDER}`, padding: "0 32px", display: "flex", gap: 0 }}>
        {tabs.map((t, i) => (
          <button key={i} onClick={() => setActiveTab(i)} style={{
            ...
            borderBottom: `2px solid ${i === activeTab ? GREEN : "transparent"}`,
            color: i === activeTab ? GREEN : "#64748B", cursor: "pointer", fontSize: 13,
            fontWeight: i === activeTab ? 700 : 400, transition: "all 0.15s"
          }}>{t}</button>
        ))}
      </div>
      ...
      {/* Footer */}
      <div style={{ borderTop: `1px solid ${BORDER}`, padding: "12px 32px", display: "flex", justifyContent: "space-between", fontSize: 11, color: "#334155" }}>
        <span>Fontes: Receita Federal · SEBRAE/DataSebrae · IBGE · MDIC · Banco Central</span>
        <span>Análise gerada em {new Date().toLocaleDateString("pt-BR")} · Carteira base: mai/26</span>
      </div>
    </div>
  );
}
